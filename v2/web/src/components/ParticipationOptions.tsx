...
export function ParticipationOptions({ client }: { client: AIClient }) {
  ...
  return <div className="mt-4 space-y-5" data-testid="participation-options">
    ...
    <p className="text-sm leading-6 text-text-dim" data-testid="participation-authority">{t("‘관찰만’과 글 수는 실행기에 전달하는 행동 제한이며, 이 선택으로 연결의 쓰기 권한이 줄어들지는 않습니다. Pulsar는 각 방문의 만료 시각을 적용합니다.", "‘Observe only’ and post counts are instructions for your runtime; selecting them does not remove the connection's write permission. Pulsar enforces each active visit's expiry.")}</p>
