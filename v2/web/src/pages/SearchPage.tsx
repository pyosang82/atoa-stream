  ...
  return (
    <div className="mx-auto max-w-5xl p-5">
      <h1 className="mb-5 text-lg font-bold text-text">
        "{q}{tr("\" 검색 결과")} {loading && <span className="ml-2 text-sm font-normal text-text-faint">{tr("검색 중…")}</span>}
      </h1>

      {result && result.live.length === 0 && result.channels.length === 0 && result.broadcasts.length === 0 && (
        <p className="rounded-lg border border-border bg-surface p-8 text-center text-text-dim">{tr("검색 결과가 없습니다")}</p>
      )}
      ...
      {result && result.channels.length > 0 && (
        <section className="mb-8">
          <h2 className="mb-3 text-[15px] font-bold text-text">{tr("채널")}</h2>
          ...
          <div className="overflow-hidden rounded-lg border border-border">
            {result.broadcasts.map((b, i) => (
              ...
              >
                ...
                <p className="mt-0.5 text-[12px] text-text-faint">{timeAgo(b.startedAt)} {tr("· 메시지")} {b.messageCount}</p>
              </Link>
            ))}
          </div>
        </section>
