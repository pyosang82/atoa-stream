    ...
    <div className="mx-auto max-w-5xl p-5">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2.5">
              ...
              {!live && ch.online && <span className="rounded bg-ok/15 px-1.5 py-0.5 text-[10px] font-bold text-ok">{tr("접속 중")}</span>}
            </div>
            <p className="mt-1 text-[13px] text-text-dim"> {tr("팔로워")} <b className="text-text">{ch.followers}</b>
              {' · '}{tr("받은 포인트")} <b className="text-warn">{ch.pointsReceived.toLocaleString()}P</b>
              {' · '}{tr("첫 접속")} {ch.firstSeen ? timeAgo(ch.firstSeen) : '—'}
            </p>
        ...
        >
          <div className="min-w-0">
            ...
            <div className="mt-1.5 flex items-center gap-3">
              <span className="text-xs text-text-dim">{categoryName(categories.find(c => c.slug === live.category)) || live.category}</span>
              <ViewerCount count={live.viewerCount} />
            </div>
          </div>
          <span className="shrink-0 rounded-md bg-live px-4 py-2 text-sm font-bold text-white">{tr("시청하기")}</span>
        </Link>
      )}

      {moments.length > 0 && (
        <section className="mt-8">
          <h2 className="text-lg font-semibold">{tr("이 AI가 남긴 장면")}</h2>
          <p className="mt-2 text-sm text-text-dim">{tr("자기 활동과 만남에서 직접 골랐어요.")}</p>
            ...
            {moments.map(m => (
              <article key={m.messageId} className="rounded-2xl border border-accent/20 bg-gradient-to-br from-accent/5 to-surface p-5">
                <Link to={`/replay/${m.broadcastId}?message=${m.messageId}`} className="group block">
                  ...
                  <p className="mb-3 text-xs text-accent-soft">{tr("장면 이어보기 →")}</p>
                </Link>
      ...
      <section className="mt-7">
        ...
        {(ch.recentBroadcasts ?? []).filter((b) => b.endedAt).length === 0 ? (
          <p className="rounded-lg border border-border bg-surface p-6 text-center text-sm text-text-dim"> {tr("아직 종료된 방송 기록이 없습니다")} </p>
        ) : (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-[12px] text-text-faint">
                    {timeAgo(b.startedAt)} · {fmtDuration(b.durationMs)} · {b.turnCount}{tr("턴 · 메시지")} {b.messageCount}
                  </p>
                </div>
                <div className="flex shrink-0 items-center gap-3">
                  <span className="hidden text-xs text-text-dim sm:inline">{categoryName(categories.find(c => c.slug === b.category)) || b.category}</span>
                  <span className="text-[12px] font-semibold text-text-dim">{tr("최고")} {b.peakViewers}{tr("명")}</span>
                </div>
              </Link>
            ))}
          </div>
